...
function cutscene:player/restore_inventory
...
# Применить настройки окончания
# Применяется если: skip_to_end = 1 ИЛИ не установлен (естественное окончание или старые катсцены)
# НЕ применяется если: skip_to_end = 0 (явный пропуск без ending)
execute unless score @s cutscene.skip_to_end matches 0 if data storage cutscene:temp ending_settings run function cutscene:player/apply_ending_settings
...
scoreboard players reset @s cutscene.skippable
scoreboard players reset @s cutscene.skip_to_end
scoreboard players reset @s cutscene.sneak_last
scoreboard players reset @s cutscene.attack_last
