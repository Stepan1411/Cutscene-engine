# Применить текст Chat к ближайшей точке
...
execute unless data storage cutscene:temp message_chat run tellraw @s {"text":"❌ Текст не установлен! Сначала введите текст.","color":"red"}
execute unless data storage cutscene:temp message_chat run return 0
...
function cutscene:edit/workspace_messages_chat_apply_macro with storage cutscene:temp

tellraw @s [{"text":"✓ Chat применен к точке ","color":"green"},{"score":{"name":"#closest_point","objective":"cutscene.temp"},"color":"yellow"}]
