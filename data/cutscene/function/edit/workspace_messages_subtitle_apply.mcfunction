# Применить текст Subtitle к ближайшей точке
...
execute unless data storage cutscene:temp message_subtitle run tellraw @s {"text":"❌ Текст не установлен! Сначала введите текст.","color":"red"}
execute unless data storage cutscene:temp message_subtitle run return 0
...
function cutscene:edit/workspace_messages_subtitle_apply_macro with storage cutscene:temp

tellraw @s [{"text":"✓ Subtitle применен к точке ","color":"green"},{"score":{"name":"#closest_point","objective":"cutscene.temp"},"color":"yellow"}]
